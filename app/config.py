...
class Settings:
    ...
    # Telegram — второй поток: лоты верхнего диапазона (>= min_competition,
    # "открытая конкуренция", 3+) уходят в ОТДЕЛЬНЫЙ чат.
    # Достаточно задать competition_chat_id — тогда в него пишет тот же основной
    # бот (он должен состоять в обоих чатах). competition_bot_token нужен, только
    # если хочется именно отдельного бота. Если competition_chat_id не задан —
    # лоты 3+ не шлём (в лог предупреждение), чтобы не смешивать с потоком менеджеров.
    ...
    # Второй, верхний диапазон отбора: лоты с числом заявок >= min_competition
    # тоже проходят. Позволяет ловить "0 и 3+", отсекая 1-2: ноль — никто не
    # заметил (лёгкий лот), 3+ — открытые торги, а 1-2 заявки часто означают
    # заранее подобранного поставщика. None = верхний диапазон выключен, работает
    # только max_applications (как раньше). Для "0 и 3+": max_applications=0,
    # min_competition=3.
    min_competition: int | None = None
    ...
    def applications_ok(self, count: int) -> bool:
        """Проходит ли лот по числу поданных заявок.

        Базовое правило — не больше max_applications (0 = только нулевая
        конкуренция). Если задан min_competition, проходят и лоты с числом
        заявок >= min_competition. Так ловится "0 и 3+" с отсечением 1-2:
        ноль — никто не заметил, 3+ — настоящие торги, а 1-2 заявки часто
        означают заранее подобранного поставщика.
        """
        if count <= self.max_applications:
            return True
        if self.min_competition is not None and count >= self.min_competition:
            return True
        return False
